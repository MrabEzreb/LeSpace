...
public class GameMenu {
	...
	public void addButton(GameButton button){
		if(button.width < menuBarWidth)
			button.setPosition(menuBarX+(menuBarWidth+(menuBarWidth-button.width)), menuBarY);
		else
	...
	private boolean checkMenuBarPressed(float mouseX, float mouseY, boolean justPressedL){
		if(justPressedL && Glbls.withinSquareBounds(mouseX, mouseY, menuBarX, menuBarY, menuBarWidth, menuBarHeight))
			return true;
		else
			return false;
	}
	...
	public void update(float mouseX, float mouseY, boolean justPressedL){
		...
		if(open){
			for(GameButton b:buttons){
				if(!b.isOutOfBounds(mouseX, mouseY, justPressedL)){
					b.doAction();
					open = false;
				}
			}
